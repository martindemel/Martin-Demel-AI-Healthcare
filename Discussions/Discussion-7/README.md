...
Recognizing that bias can be embedded in the measurement device itself (pulse oximetry overestimating SpO2 in neonates) — not just in the training data — was a difficult but important insight. It showed that bias mitigation must begin before the algorithm is even built.
